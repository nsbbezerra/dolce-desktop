...
export default function SaveRevenue() {
  const { data, error } = useFetch("/revenuesDependets");
  ...
  async function register(e) {
    ...
    if (status === "") {
      handleValidator("status", "Selecione um status para a receita");
      return false;
    }
    if (movimentation === "") {
      handleValidator(
        "movimentation",
        "Selecione ums movimentação para a receita"
      );
      return false;
    }
    setValidators([]);
